...
type GreetServiceClient interface {
	...
	SayHelloClientStreming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloClientStremingClient, error)
...
func (c *greetServiceClient) SayHelloClientStreming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloClientStremingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[1], "/greet_service.GreetService/SayHelloClientStreming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayHelloClientStremingClient{stream}
	return x, nil
}

type GreetService_SayHelloClientStremingClient interface {
	Send(*HelloRequest) error
	CloseAndRecv() (*MessagesList, error)
	grpc.ClientStream
}

type greetServiceSayHelloClientStremingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayHelloClientStremingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayHelloClientStremingClient) CloseAndRecv() (*MessagesList, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(MessagesList)
...
type GreetServiceServer interface {
	...
	SayHelloClientStreming(GreetService_SayHelloClientStremingServer) error
...
func (UnimplementedGreetServiceServer) SayHelloClientStreming(GreetService_SayHelloClientStremingServer) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloClientStreming not implemented")
}
...
func _GreetService_SayHelloClientStreming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayHelloClientStreming(&greetServiceSayHelloClientStremingServer{stream})
}

type GreetService_SayHelloClientStremingServer interface {
	SendAndClose(*MessagesList) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayHelloClientStremingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayHelloClientStremingServer) SendAndClose(m *MessagesList) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayHelloClientStremingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayHelloClientStreming",
			Handler:       _GreetService_SayHelloClientStreming_Handler,
			ClientStreams: true,
		},
		{
